...
namespace nocturne::editor
{
    namespace
    {
        ...
        LRESULT CALLBACK ButtonProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
        {
            ...
            case WM_NCCREATE:
            {
                ...
                if (init)
                    created->kind = init->kind;

                if (cs->lpszName)
                    created->label = cs->lpszName;
                ...
                if (state)
                {
                    ...
                    else if (state->kind == EditorButtonKind::Success)
                    {
                        bg = state->pressed   ? Blend(c.buttonBg, c.windowBg, 24)
                             : state->hovered ? Blend(c.buttonBg, c.success, 10)
                                              : c.buttonBg;
            ...
            case WM_MOUSEMOVE:
                if (state)
                {
                    ...
                    if (state->dragging)
                    {
                        ...
                        const int maxPos = ScrollMaxPos(*state);
                        NotifyScroll(hwnd, *state, state->info.minimum + static_cast<int>(t * (maxPos - state->info.minimum) + 0.5));
                    }
                }
                return 0;
            case WM_MOUSELEAVE:
        ...
        LRESULT CALLBACK InputProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
        {
            ...
            case WM_NCCREATE:
            {
                ...
                if (init && init->placeholder)
                    created->placeholder = init->placeholder;
                SetWindowLongPtrW(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(created));
                return TRUE;
            }
            case WM_CREATE:
    ...
    {
        return RegisterClass(instance, kButtonClass, ButtonProc, LoadCursorW(nullptr, IDC_HAND)) && RegisterClass(instance, kScrollClass, ScrollProc, LoadCursorW(nullptr, IDC_ARROW)) && RegisterClass(instance, kTableClass, TableProc, LoadCursorW(nullptr, IDC_ARROW)) && RegisterClass(instance, kTreeClass, TreeProc, LoadCursorW(nullptr, IDC_ARROW)) && RegisterClass(instance, kInputClass, InputProc, LoadCursorW(nullptr, IDC_IBEAM));
    }
    ...
    {
        if (button)
            SendMessageW(button, WM_NOC_BUTTON_ACTIVE, active ? TRUE : FALSE, 0);
    }
