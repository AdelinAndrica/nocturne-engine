    ...
    namespace
    {
        ...
        private:
            ...
            HBITMAP Rasterize(EditorIconId icon, int width, int height, COLORREF color)
            {
                ...
                HRESULT hr = wicFactory_->CreateBitmap(static_cast<UINT>(width), static_cast<UINT>(height),
                ...
                hr = d2dContext_->CreateBitmapFromWicBitmap(wicBitmap.Get(), &properties, &target);
                if (FAILED(hr)) return nullptr;
                ...
                svgContext_->Clear(D2D1::ColorF(0.0f, 0.0f));
                ...
                UINT byteCount = 0;
                BYTE* source = nullptr;
                if (FAILED(lock->GetStride(&stride)) || FAILED(lock->GetDataPointer(&byteCount, &source)) || !source)
                    return nullptr;
                ...
                for (int y = 0; y < height; ++y)
                {
                    std::memcpy(dst + static_cast<size_t>(y) * rowBytes,
                        source + static_cast<size_t>(y) * stride, rowBytes);
                }
                return bitmap;
            }
