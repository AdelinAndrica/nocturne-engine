...
source_files: [".github/workflows/release-package.yml",".github/scripts/package-release.ps1","Schemas/release.schema.json","Website/scripts/promote-release.mjs","Website/src/data/releases.json","Website/src/data/release-schema.mjs","Website/src/pages/download.astro"]
...
6. promoted validated release records into a reviewed website-manifest pull request;
7. merged that reviewed manifest change before the Downloads page claims support.
...
Public Downloads still require a real tagged publication plus review and merge of the generated manifest pull request.
...
The website manifest remains a separate reviewed source.

A GitHub Release existing by itself does not cause the website to claim support automatically.

After the release assets are published, the tag-only job:

`Prepare Downloads manifest PR`

performs the normal website publication preparation:

1. downloads the validated architecture release records from the same workflow run;
2. checks out `master`;
3. runs `Website/scripts/promote-release.mjs`;
4. validates the resulting manifest;
5. builds the Downloads page;
6. uploads the candidate `releases.json` as workflow evidence;
7. creates or updates a `release-manifest/<tag>` branch;
8. opens a pull request against `master`.

The job has no auto-merge step.

The Downloads page changes only after that PR is reviewed and merged.

The promotion tool remains directly callable for recovery, local validation or deliberate maintenance:
...
If manifest preparation or PR creation fails after the GitHub Release exists, the website manifest remains unchanged. This preserves artifact honesty: a partial publication cannot silently alter the public support matrix.

**Design choice (not directly from the book):** normal manifest promotion is automated up to a source-controlled review boundary, while merge remains an explicit human action.
...
- release-record promotion fails;
- the reviewed manifest pull request cannot be prepared.

A failure after GitHub Release publication does not retroactively delete the release assets; it prevents the website from claiming them until the manifest-review path succeeds.
