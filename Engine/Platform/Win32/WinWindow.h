...
namespace noc
{
    namespace platform
    {
        ...
        // Design choice (not directly from the book): optional application-level message sink.
        // This keeps WinWindow as the owner of the native window procedure while permitting
        // tools such as Nocturne Editor to layer UI behavior on top without forking platform code.
        struct IWindowMessageSink
        {
            virtual ~IWindowMessageSink() = default;
            virtual bool OnWindowMessage(void* hwnd, uint32_t msg, uintptr_t wParam,
                intptr_t lParam, intptr_t& result) = 0;
        };
    }
    ...
    public:
        ...
        // Phase 13: optional tool/application UI extension point.
        void SetMessageSink(platform::IWindowMessageSink* sink) { messageSink_ = sink; }
    ...
    private:
        ...
        platform::IWindowMessageSink* messageSink_ = nullptr;
    };
}
