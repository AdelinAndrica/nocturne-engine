...
namespace noc
{
    ...
    LRESULT CALLBACK WinWindow::WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
    {
        ...
        // Phase 13 extension point: give the tool/application layer first chance to
        // consume UI-specific messages while WinWindow remains the owner of WndProc.
        if (win && win->messageSink_)
        {
            intptr_t toolResult = 0;
            if (win->messageSink_->OnWindowMessage(
                hWnd,
                ...
                toolResult))
            {
                return static_cast<LRESULT>(toolResult);
            }
        }
